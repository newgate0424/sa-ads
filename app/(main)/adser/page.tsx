                                        ...
                                        <TableBody>
                                            {teamsInGroup.sort((a, b) => teamNames.indexOf(a.team_name) - teamNames.indexOf(b.team_name)).map(team => (
                                                <TableRow key={team.team_name}>
                                                    ...
                                                    {showBreakdown && <>
                                                        {Object.keys(allConfigurableFields).filter(key => allConfigurableFields[key as keyof typeof allConfigurableFields].unit === '%' && !['wasted_inquiries', 'net_inquiries'].includes(key)).map(key => (
                                                            <TableCell key={key} className="text-center"><span style={getCellStyle(team.team_name, key as keyof typeof allConfigurableFields, team[key], team.total_inquiries)}><BreakdownCell value={team[key]} total={team.total_inquiries} /></span></TableCell>
                                                        ))}
                                                    </>}
                                                </TableRow>
                                            ))}
