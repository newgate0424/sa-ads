...
/** ✅ คลาสกลางสำหรับ "กรอบฟิลเตอร์" ทั้งฝั่งตารางและกราฟ
 * - ทำให้พื้นหลัง/เส้นขอบ/ความสูง/ระยะขอบใน เหมือนกันเป๊ะ
 * - ถ้าอยากพื้นหลังขาวสนิท เปลี่ยน bg-muted/50 -> bg-background
 */
        ...
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={data} margin={{ top: 5, right: 30, left: -10, bottom: 20 }}>
            ...
            {chartType === 'cover' && groupName && coverTargets[groupName] && (
              <ReferenceLine y={coverTargets[groupName]} stroke="#ef4444" strokeDasharray="6 6" strokeWidth={1}>
                <Label value={`${coverTargets[groupName]}`} position="right" fill="#ef4444" fontSize={11} fontWeight="nomal" />
              </ReferenceLine>
            )}
...
export default function AdserPage() {
  // ... (State declarations and SWR hooks remain the same) ...
  ...
  const [graphView, setGraphView] = useState<'daily' | 'monthly'>(() => {
    if (typeof window === 'undefined') return 'daily';
    const savedView = localStorage.getItem('graphView');
    return (savedView === 'daily' || savedView === 'monthly') ? savedView : 'daily';
  });
  ...
  useEffect(() => {
    ...
    const getDateRangeFromStorage = (key: string): DateRange => {
      const savedDate = localStorage.getItem(key);
      if (savedDate) {
        try {
          const parsed = JSON.parse(savedDate);
          if (parsed.from && parsed.to) {
            return { from: dayjs(parsed.from).toDate(), to: dayjs(parsed.to).toDate() };
          }
        } catch (e) { console.error(`Failed to parse date range from ${key}:`, e); }
      }
      return defaultThisMonth;
    };
    setTableDateRange(getDateRangeFromStorage('dateRangeFilterAdserTable'));
  }, []);

  useEffect(() => { if (tableDateRange) { localStorage.setItem('dateRangeFilterAdserTable', JSON.stringify(tableDateRange)); } }, [tableDateRange]);

  // --- 🟢 [แก้ไข] ปรับปรุง Logic การประมวลผลข้อมูลกราฟทั้งหมด ---
  useEffect(() => {
    ...
    const transformData = (dataKey: keyof TeamMetric, monthlyAgg: 'sum' | 'avg' | 'last') => {
      ...
      // --- 🟢 [เพิ่ม] กรองข้อมูลที่เป็นวันในอนาคตออกไป ---
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
        ...
        <div className="flex flex-col sm:flex-row gap-2">
          ...
          <div>
            ...
            {/* ✅ ห่อ DateRangePicker ด้วยกรอบฟิลเตอร์กลาง ให้พื้นหลัง/ขอบ/ความสูงตรงกัน */}
          ...
          <div className="flex flex-col items-center sm:items-start">
            ...
            {/* ⛳ เดิมเป็น: className="flex items-center rounded-md border border-input bg-transparent p-1 gap-1"
                ✅ เปลี่ยนเป็น filterFrameClass ให้ตรงกับฝั่งตาราง */}
            <div className={filterFrameClass}>
              {graphView === 'daily' && (
                <Select value={String(graphMonth)} onValueChange={(v) => setGraphMonth(Number(v))}>
                  {/* ⛳ เดิม h-8 -> ✅ h-9 เพื่อให้ความสูงภายในกรอบเท่ากัน */}
